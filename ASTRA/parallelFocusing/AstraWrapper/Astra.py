...
class Astra:
    ...
    def __init__(self, settingsFile):
        ...
        bash_command = "source /opt/intel/oneapi/setvars.sh\n"
        self.process = Popen(
            ['/bin/bash'],
            stdout=PIPE, 
            stderr=PIPE, 
        ...
        self.process.stdin.write(bash_command)
    ...
    def changeMom(self,pz, xAngle=-1, yAngle=-1, xoff=-1, yoff=-1,sig_Ekin =-1, sig_z = -1 ): 
        ...
        try:
            ...
            self.process.stdin.write("./generator " + self.fileName + "\n")
            self.process.stdin.flush()
    ...
    def runRef(self, D1, D2, D3, D4, hardEnd,momZ, moreData):
        ...
        if moreData:
            ...
            for i in range(len(inputDataName)):
                ...
                #if i == 0 and not self.isRef0Straight(currentData[-1][7], currentData[-1][8]):
    ...
    def findInfo(self,D1,D2,D3,D4, hardEnd, momZ):
        ...
        if accept == 1:
            print(f"Cannot check angle acceptance for {D1, D2, D3, D4, hardEnd,momZ}.")
            return 1
        ...
        #look for closest in the z direction
        closest= 0.1
