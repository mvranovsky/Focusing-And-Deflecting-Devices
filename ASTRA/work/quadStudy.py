...
def plotAll3QuadTypes():
    # function that plots the same x ray, but with 3 different quadTypes: top hats, astra fringe fields, gradient profiles
    try:
        astra.quadType(0)
...
def func(D, Pz):
    #function that runs astra, evaluates the output and returns the function value
    data = []
    try:
...
def func1(D,D1, Pz):
    #same as func() only takes D1 as parameter and does not vary it
    data = []
    try:
...
def tripletFocusing(Pz, D1 = None , defaultFieldType=1, limitValue = 0.0001, FFFactor = 1 ):
    # function that looks for a solution for a specific Pz and/or specific D1
    # the field types are: 0 = top hat, 1 = astra fringe fields, 2 gradient profiles from Sumit's fits
    # limitValue is the minimum function value. If the function value is smaller-> solution found, if not, solution was not found
    # FFFactor is Fringe Field Factor. factor that defines the minimum for D1,D2,D3
    method = "COBYLA"
    method = "Powell"
    tolerance = 1e-10
    ...
    try:
        # get the properties of the focusing
...
def study(solution, fields , key , Vals, description):
    # this function was used for quad1Study. It generates a beam, runs it through with the offset and saves all the output to a root file
    # this root file will then be opened and plots are created. 
...
def changeInput(filename, tag, newVal):
    #change initial parameter of a ray
    line = []
...
def linear_function(x, slope): #function for fitting, constant = 0
    return slope * x

def linear_function_with_C(x,slope, C): #function for fitting ,constant = C
    return slope * x + C

def quadratic_function(x, pa2, pa1, pa0): # fit with quadratic function
    return x*x*pa2 + pa1*x1 + pa0

def fit(xVals, x1, y1, x2, y2, key):
    #main function for fitting slopes
    x = np.array(xVals)
    x1 = np.array(x1)
    y1 = np.array(y1)
    x2 = np.array(x2)
